...
export const formSchema = z
  .object({
    ...
    condition_mode: z.enum(["concat", "replace"]).default("concat"),
    quality: z.enum(["low", "medium", "high"]).default("medium"),
    geometry_file_format: z.enum(["glb", "obj"]).default("glb"),
    use_hyper: z.boolean().default(false),
    tier: z.enum(["Regular", "Premium"]).default("Regular"),
    TAPose: z.boolean().default(false),
    material: z.enum(["PBR", "Basic"]).default("PBR"),
    options: z
      .object({
        style: z.enum(["realistic", "stylized", "cartoon", "abstract"]).default("realistic"),
